...
- A localhost-only local report runtime adapter exists for evidence-cited report generation tests, but it only talks to a user-managed local model service, refuses oversized prompts, and no real local LLM runtime has been smoke-tested. The desktop report panel does not download models or start a model server.
