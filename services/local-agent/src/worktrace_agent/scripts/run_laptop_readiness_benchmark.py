...
from worktrace_agent.performance.laptop_readiness import (
    DEFAULT_LAPTOP_READINESS_DURATION_SECONDS,
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run a short local WorkTrace recorder benchmark and emit a safe aggregate report."
        )
    )
    parser.add_argument(
        "--duration-seconds",
        type=float,
        default=DEFAULT_LAPTOP_READINESS_DURATION_SECONDS,
        help="Benchmark duration in seconds. Use 300 or 600 for portfolio evidence.",
    )
    parser.add_argument(
